...
func TestGetListWebinarParticipantsQoS(t *testing.T) {
	...
	webinar, err := zoom.GetListWebinarParticipantsQoS("8722824397")
	if err != nil {
		t.Error(err)
	}

	assert.NotNil(t, webinar, "results are empty")
	assert.Equal(t, len(webinar.WebinarParticipants), 0, "participants not found")
}
